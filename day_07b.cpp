...
const int total_space = 70'000'000;
const int min_free_space = 30'000'000;
...
class Directory
{
    public:
        ...
        int recurse_find_min_size(int min_dir_size, int min_so_far=-1)
        {
            int dir_size = get_size();
            if (dir_size >= min_dir_size && ( (min_so_far == -1) || (dir_size < min_so_far) ))
                min_so_far = dir_size;
            
            //cout << "(before recursion) " << name << ": " << dir_size << ", " << min_so_far << "\n";
            
            for (auto it = subdirs.begin(); it != subdirs.end(); it ++) {
                min_so_far = (*it)->recurse_find_min_size(min_dir_size, min_so_far);
            }

            //cout << "(after recursion)  " << name << ": " << dir_size << ", " << min_so_far << "\n";
...
int main(int argc, char *argv[])
{
    ...
    int min_deletion_size = (p_root_dir->get_size() - (total_space - min_free_space));
    if (min_deletion_size < 0)
        min_deletion_size = 0;
    int target_size = p_root_dir->recurse_find_min_size(min_deletion_size);
